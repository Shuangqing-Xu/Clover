...
def get_dataloader_cifar10(datadir, train_bs, test_bs, dataidxs=None,test_idxs=None, cache_train_data_set=None,cache_test_data_set=None):
...
def load_partition_data_cifar10( data_dir, partition_method, partition_alpha, client_number, batch_size, logger):
    ...
    for i in range(len(net_dataidx_map)):
        val_dataidx_map.append(random.sample(net_dataidx_map[i], int(0.1 * len(net_dataidx_map[0]))))
    ...
    for client_idx in range(client_number):
        ...
        val_data_local, _ = get_dataloader_cifar10( data_dir, batch_size, batch_size,
                                                 val_dataidxs,test_dataidxs[client_idx] ,cache_train_data_set=cache_train_data_set,cache_test_data_set=cache_test_data_set)
        local_data_num = len(train_data_local.dataset)
        data_local_num_dict[client_idx] = local_data_num
        logger.info("client_idx = %d, local_train_number = %d, local_val_number = %d, local_test_number = %d" % (client_idx, local_data_num, len(val_data_local.dataset), len(test_data_local.dataset)))
    ...
    return None, None, None, None, \
           data_local_num_dict, train_data_local_dict, val_data_local_dict, test_data_local_dict, traindata_cls_counts
